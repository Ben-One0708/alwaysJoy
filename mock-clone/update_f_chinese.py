...
try:
    with open('D-vocab-review.html', 'r', encoding='utf-8') as f:
        ...
        matches = re.findall(r'\{ english: "([^"]+)", meaning: "([^"]+)", module: "([^"]+)" \}', content)
        for eng, mean, mod in matches:
            d_words_dict[eng.lower()] = mean
    print(f"從 D 組載入了 {len(d_words_dict)} 個單字作為參考", file=sys.stderr)
except:
    print("無法讀取 D 組單字", file=sys.stderr)
...
try:
    with open('E-vocab-review.html', 'r', encoding='utf-8') as f:
        ...
        matches = re.findall(r'\{ english: "([^"]+)", meaning: "([^"]+)", module: "([^"]+)" \}', content)
        for eng, mean, mod in matches:
            e_words_dict[eng.lower()] = mean
    print(f"從 E 組載入了 {len(e_words_dict)} 個單字作為參考", file=sys.stderr)
except:
    print("無法讀取 E 組單字", file=sys.stderr)

# 讀取 F 組單字
with open('words_f.json', 'r', encoding='utf-8') as f:
    words = json.load(f)
...
for i, word in enumerate(words):
    ...
    if not has_chinese or len(current_meaning.strip()) < 2:
        ...
        elif english.lower() in d_words_dict:
            new_meaning = d_words_dict[english.lower()]
            reused_count += 1
            if (i + 1) % 50 == 0:
                print(f"[{i+1}/{len(words)}] 重用 D 組: {english} -> {new_meaning}", file=sys.stderr)
        elif english.lower() in e_words_dict:
            new_meaning = e_words_dict[english.lower()]
            reused_count += 1
            if (i + 1) % 50 == 0:
                print(f"[{i+1}/{len(words)}] 重用 E 組: {english} -> {new_meaning}", file=sys.stderr)
        else:
            # 查詢線上字典
            if (i + 1) % 20 == 0:
    ...
    # 每100個單字保存一次
    if (i + 1) % 100 == 0:
        with open('words_f_updated.json', 'w', encoding='utf-8') as f:
...
print(f"\n完成！", file=sys.stderr)
print(f"更新了 {updated_count} 個單字 (其中 {reused_count} 個從 C/D/E 組重用)", file=sys.stderr)
if failed_words:
