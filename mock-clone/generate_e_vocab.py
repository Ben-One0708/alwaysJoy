...
# 讀取 D-vocab-review.html 作為模板
with open('D-vocab-review.html', 'r', encoding='utf-8') as f:
    template = f.read()

# 讀取更新後的 E 組單字
with open('words_e_updated.json', 'r', encoding='utf-8') as f:
    words = json.load(f)
...
# 替換模板中的內容
# 1. 替換標題
template = template.replace('D組必讀單字複習', 'E組必讀單字複習')
template = template.replace('1~10冊必背應用字彙', '1~13冊必背應用字彙')
...
template = template.replace("const STORAGE_KEY = 'D_vocab_learned';", "const STORAGE_KEY = 'E_vocab_learned';")

# 寫入新文件
with open('E-vocab-review.html', 'w', encoding='utf-8') as f:
    f.write(template)

print(f"已生成 E-vocab-review.html，包含 {len(words)} 個單字")
