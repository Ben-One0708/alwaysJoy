...
# 讀取 E 組單字
with open('words_e.json', 'r', encoding='utf-8') as f:
    words = json.load(f)
...
for i, word in enumerate(words):
    ...
    # 每100個單字保存一次
    if (i + 1) % 100 == 0:
        with open('words_e_updated.json', 'w', encoding='utf-8') as f:
...
print(f"\n完成！", file=sys.stderr)
print(f"更新了 {updated_count} 個單字 (其中 {reused_count} 個從 C/D 組重用)", file=sys.stderr)
if failed_words:
